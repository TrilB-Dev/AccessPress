<?php
/**
 * SettingsPlugins class for AccessPress plugin.
 *
 * @package AccessPress
...
final class SettingsPlugins {
	...
	public function can_view_settings_page( string $slug ): bool {
		...
		$capability = SanitizationHelper::key(
			$page['capability'] ?? 'accesspress_settings_plugins_int_view'
		);
		...
		<tr>
			...
			<td>
				<?php foreach ( $page['fields'] as $field ) : ?>
					...
					$name    = 'accesspress_' . SanitizationHelper::key( $page['slug'] ) . '[' . $key . ']';
					...
					<div class="mb-3">
						...
						<?php elseif ( 'custom' === $type ) : ?>
							<?php $render = $field['render'] ?? null; if ( is_callable( $render ) ) { call_user_func( $render, $value, $name, 'accesspress-' . $key ); } ?>
						<?php else : ?>
	...
	public function render( string $tab ): void {
		...
		$this->render_accesspress_plugins();
	}
	/**
	...
	/**
	 * Render the AccessPress plugins section.
	 *
	 * @since 1.0.0
	 */
	private function render_accesspress_plugins(): void {
		?>
		<div class="row g-4">
			<?php foreach ( Plugins::get_instance()->get_registered_plugins() as $plugin ) : ?>
				<?php if ( $plugin instanceof PluginInterface && $this->can_view_plugin( $plugin ) ) : ?>
					<?php $this->render_accesspress_plugin_card( $plugin ); ?>
				<?php endif; ?>
			<?php endforeach; ?>
		</div>
		<?php
	...
	private function render_third_party_plugins(): void {
		...
		<div class="row g-4">
			<?php foreach ( get_plugins() as $file => $plugin ) : ?>
				<?php if ( function_exists( 'plugin_basename' ) && plugin_basename( ACCESSPRESS_FILE ) === $file ) : ?>
					<?php continue; ?>
				<?php endif; ?>
	 ...
	 */
	private function render_accesspress_plugin_card( $plugin ): void {
		...
		<div class="col-12 col-md-6 col-xl-4 d-flex">
			<article class="card accesspress-plugin-card shadow-sm h-100 w-100">
				<div class="card-header d-flex align-items-center gap-2">
					<?php /* translators: %s is the plugin name. */ ?>
					<?php
					echo FormFieldHelper::switch(
						'accesspress-plugin-status',
						'1',
						'',
						array(
							'id'               => 'accesspress-plugin-status-' . SanitizationHelper::key( $plugin->get_slug() ),
							'checked'          => $enabled,
							'disabled'         => ! $can_edit,
							'data-accesspress-plugin-toggle' => 'true',
							'data-plugin-slug' => $plugin->get_slug(),
							'aria-label'       => sprintf(
								/* translators: %s is the plugin name. */
								__( 'Enable %s', 'accesspress' ),
								$plugin->get_name()
							),
						)
					);
				...
				<div class="card-body d-flex flex-column">
					...
					<p class="card-text mb-2"><span class="text-secondary"><?php esc_html_e( 'Author:', 'accesspress' ); ?></span> <?php echo esc_html( $plugin->get_author() ); ?></p>
					<p class="card-text mb-2"><span class="text-secondary"><?php esc_html_e( 'Version:', 'accesspress' ); ?></span> <?php echo esc_html( $plugin->get_version() ); ?></p>
					<p class="card-text mb-3"><span class="text-secondary"><?php esc_html_e( 'Docs:', 'accesspress' ); ?></span> <a href="<?php echo esc_url( $plugin->get_uri() ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'View documentation', 'accesspress' ); ?></a></p>
					<?php if ( ! empty( $settings_page['fields'] ) ) : ?>
						<?php
						echo FormFieldHelper::button(
							__( 'Settings', 'accesspress' ),
							array(
								'type'           => 'button',
	...
	private function render_plugin_settings_modal(
		...
		?>
		<div class="modal fade accesspress-plugin-settings-modal" id="<?php echo esc_attr( $modal_id ); ?>" tabindex="-1" aria-labelledby="<?php echo esc_attr( $modal_id . '-label' ); ?>" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
				<div class="modal-content">
					<div class="modal-header">
						...
						<?php echo FormFieldHelper::button(
							...
							array(
								...
								'aria-label'     => __( 'Close', 'accesspress' ),
								'raw'            => true,
							),
						); ?>
					</div>
					<div class="modal-body">
						<section class="accesspress-plugin-modal-info mb-4" aria-labelledby="<?php echo esc_attr( $modal_id . '-info' ); ?>">
							<h3 class="h6" id="<?php echo esc_attr( $modal_id . '-info' ); ?>"><?php esc_html_e( 'Plugin information', 'accesspress' ); ?></h3>
							<p class="text-secondary mb-3"><?php echo esc_html( $plugin->get_description() ); ?></p>
							<dl class="row mb-0 small">
								<dt class="col-sm-3 text-secondary"><?php esc_html_e( 'Author', 'accesspress' ); ?></dt>
								<dd class="col-sm-9"><?php echo esc_html( $plugin->get_author() ); ?></dd>
								<dt class="col-sm-3 text-secondary"><?php esc_html_e( 'Version', 'accesspress' ); ?></dt>
								<dd class="col-sm-9"><?php echo esc_html( $plugin->get_version() ); ?></dd>
								<dt class="col-sm-3 text-secondary"><?php esc_html_e( 'License', 'accesspress' ); ?></dt>
								<dd class="col-sm-9 mb-0"><?php echo esc_html( $plugin->get_license() ); ?></dd>
							</dl>
						</section>
						<form class="accesspress-plugin-settings-form" data-plugin-settings-form data-plugin-slug="<?php echo esc_attr( $plugin->get_slug() ); ?>" data-internal-accesspress-fields>
					...
					<div class="modal-footer">
						<?php echo FormFieldHelper::button( __( 'Cancel', 'accesspress' ), array( 'class' => 'btn-secondary', 'type' => 'button', 'data-bs-dismiss' => 'modal' ) ); ?>
						<?php if ( $can_edit ) : ?>
							<?php echo FormFieldHelper::button( __( 'Save', 'accesspress' ), array( 'class' => 'btn-primary', 'type' => 'button', 'data-plugin-settings-save' => 'true' ) ); ?>
						<?php endif; ?>
					</div>
				</div>
			</div>
	...
	private function can_view_plugin( PluginInterface $plugin ): bool {
		$capability = $this->is_internal_plugin( $plugin )
			? 'accesspress_settings_plugins_int_view'
			: 'accesspress_settings_plugins_ext_view';
	...
	private function can_edit_plugin( PluginInterface $plugin ): bool {
		$capability = $this->is_internal_plugin( $plugin )
			? 'accesspress_settings_plugins_int_edit'
			: 'accesspress_settings_plugins_ext_edit';
		return current_user_can( $capability );
	}
	/**
	 * Check if a plugin is an internal AccessPress plugin.
	 *
	 * @param PluginInterface $plugin The plugin instance.
	...
	private function render_plugin_settings_fields( array $settings_page, array $values, string $prefix ): void {
		...
		if ( 'table' === $layout ) {
			?>
			<div class="accesspress-plugin-settings-fields accesspress-plugin-settings-fields-table"><table class="table align-middle"><tbody>
			<?php
		} else {
			?>
			<div class="accesspress-plugin-settings-fields accesspress-plugin-settings-fields-box">
			<?php
		}
		...
		foreach ( $settings_page['fields'] as $field ) {
			...
			if ( ! empty( $field['visible_when'] ) && is_array( $field['visible_when'] ) ) {
				$wrapper_attributes['data-accesspress-visible-when'] = wp_json_encode( $field['visible_when'] );
			}
			...
			if ( 'table' === $layout ) {
				...
				<tr<?php echo $wrapper_attributes ? ' ' . $wrapper_attributes : ''; ?>>
					...
					<td>
			<?php } else { ?>
				<article class="accesspress-plugin-settings-field card h-100"<?php echo $wrapper_attributes ? ' ' . $wrapper_attributes : ''; ?>>
					<div class="card-body">
						<div class="accesspress-plugin-settings-field-header d-flex align-items-start justify-content-between gap-3">
				...
				?>
						...
						<?php if ( ! empty( $field['description'] ) ) { ?>
							<p class="accesspress-plugin-settings-field-description text-secondary mb-3">
		...
		<div class="col-12 col-md-6 col-xl-6 d-flex">
			<article class="card accesspress-plugin-card shadow-sm h-100 w-100">
				<div class="card-header d-flex align-items-center gap-2">
					...
					<?php
					echo FormFieldHelper::switch(
						'accesspress-third-party-status',
						'1',
						'',
						array(
							'id'         => 'accesspress-third-party-status-' . SanitizationHelper::key( $file ),
							'checked'    => $active,
							'disabled'   => true,
							'aria-label' => sprintf(
								/* translators: %s is the plugin name. */
				...
				<div class="card-body d-flex flex-column">
					...
					<p class="card-text text-secondary mt-3">
						<?php echo esc_html( $plugin['Description'] ?? __( 'No description provided.', 'accesspress' ) ); ?>
					</p>
					<p class="card-text mb-2">
						<span class="text-secondary">
							<?php esc_html_e( 'Author:', 'accesspress' ); ?>
						</span>
						<?php echo esc_html( $plugin['AuthorName'] ?? wp_strip_all_tags( $plugin['Author'] ?? __( 'Unknown', 'accesspress' ) ) ); ?>
					</p>
					<p class="card-text mb-2">
						<span class="text-secondary">
							<?php esc_html_e( 'Version:', 'accesspress' ); ?>
						</span>
						<?php echo esc_html( $plugin['Version'] ?? __( 'Unknown', 'accesspress' ) ); ?>
					</p>
					<p class="card-text mb-3">
						<span class="text-secondary">
							<?php esc_html_e( 'Docs:', 'accesspress' ); ?>
						</span> 
						<?php if ( ! empty( $plugin['PluginURI'] ) ) : ?>
							<a href="<?php echo esc_url( $plugin['PluginURI'] ); ?>" target="_blank" rel="noopener noreferrer">
								<?php esc_html_e( 'View documentation', 'accesspress' ); ?>
							</a>
						<?php else : ?>
							<?php esc_html_e( 'Not available', 'accesspress' ); ?>
						<?php endif; ?>
					</p>
					<a href="<?php echo esc_url( admin_url( 'plugins.php' ) ); ?>" class="btn btn-primary mt-auto">
						<?php esc_html_e( 'Settings', 'accesspress' ); ?>
					</a>
				</div>
			</article>
		</div>
