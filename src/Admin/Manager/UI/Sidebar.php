<?php
/**
 * Sidebar UI component for AccessPress admin pages.
 *
 * @package AccessPress
...
final class Sidebar {
	...
	public static function render(): void {
		$current = RequestHelper::get_key( 'page', 'accesspress' );
		...
		<aside class="col-12 col-lg-auto accesspress-sidebar-column">
			<div class="accesspress-sidebar position-sticky" style="top: 32px;">
				...
				<nav aria-label="<?php esc_attr_e( 'AccessPress admin navigation', 'accesspress' ); ?>">
					<a class="accesspress-sidebar-link <?php echo 'accesspress' === $current ? 'active' : ''; ?>" href="<?php echo esc_url( admin_url( 'admin.php?page=accesspress' ) ); ?>">
						<?php echo self::render_icon_markup( 'fa-solid fa-house' ); ?><?php esc_html_e( 'Dashboard', 'accesspress' ); ?>
					</a>
					<div id="accesspress-sidebar-groups">
						<?php foreach ( $groups as $key => $group ) : ?>
							<?php $expanded = self::group_is_expanded( $key, $group, $current ); ?>
							<div class="accesspress-sidebar-group">
								<h3 class="accesspress-sidebar-group-heading">
										<?php echo FormFieldHelper::button(
											...
											array(
												'class'          => 'accesspress-sidebar-link accesspress-sidebar-group-link border-0 bg-transparent w-100 text-start ' . ( $expanded ? '' : 'collapsed' ),
												'type'           => 'button',
												'data-bs-toggle' => 'collapse',
												'data-bs-target' => '#accesspress-group-' . esc_attr( $key ),
												'aria-expanded'  => $expanded ? 'true' : 'false',
												'aria-controls'  => 'accesspress-group-' . esc_attr( $key ),
												'raw'            => true,
											)
										); ?>
								</h3>
								<div id="accesspress-group-<?php echo esc_attr( $key ); ?>" class="collapse <?php echo $expanded ? 'show' : ''; ?>">
	...
	private static function render_icon_markup( string $icon, bool $with_spacing = false ): string {
		...
		if ( preg_match( '/^(https?:)?\/\//i', $icon ) || preg_match( '/\.(svg|png|jpg|jpeg|webp)(\?.*)?$/i', $icon ) ) {
			return sprintf(
				'<span class="accesspress-sidebar-icon accesspress-sidebar-icon-image" aria-hidden="true"><img src="%1$s" alt="" loading="lazy"%2$s /></span>',
				esc_url( $icon ),
		...
		return sprintf(
			'<span class="accesspress-sidebar-icon" aria-hidden="true"><i class="%1$s%2$s"></i></span>',
			esc_attr( $icon ),
			$with_spacing ? ' me-2' : ''
		);
	}
