<?php
/**
 * AccessPress - FontAwesome Assets
 *
 * @package AccessPress
 * @since 1.0.0
 */
namespace AccessPress\Includes\Plugins\FontAwesome\Assets;
...
use AccessPress\Includes\Plugins\FontAwesome\Includes\Settings\Settings as FontAwesomeSettings;
...
final class Assets {
	 ...
	 * duplicate AccessPress-managed FontAwesome bundle.
	 *
	...
	public function enqueue_wordpress_fontawesome_handles( string $hook_suffix = '' ): void {
		...
		if ( false === strpos( $hook_suffix, 'accesspress' ) && 0 !== strpos( $page, 'accesspress' ) ) {
			return;
		}
	...
	public function register_icon_picker_assets( array $assets, string $context = 'admin' ): array {
		...
		$assets['styles'][]  = array(
			'handle' => 'accesspress-fontawesome-icon-picker',
			'src'    => ACCESSPRESS_URL . 'src/Includes/Plugins/FontAwesome/Assets/dist/css/fontawesome.icon-picker.css',
		);
		$assets['scripts'][] = array(
			'handle'    => 'accesspress-fontawesome-icon-picker',
			'src'       => ACCESSPRESS_URL . 'src/Includes/Plugins/FontAwesome/Assets/dist/js/fontawesome.icon-picker.js',
			'deps'      => array( 'jquery' ),
			'in_footer' => true,
			'localize'  => array(
				'object_name' => 'accesspress_fa_picker',
				'data'        => array(
					'ajax_url' => admin_url( 'admin-ajax.php' ),
					'nonce'    => wp_create_nonce( 'accesspress_fontawesome_picker' ),
					'strings'  => array(
						'search_placeholder' => __( 'Search icons...', 'accesspress' ),
						'no_icons_found'     => __( 'No icons found', 'accesspress' ),
						'loading'            => __( 'Loading...', 'accesspress' ),
						'select_icon'        => __( 'Select Icon', 'accesspress' ),
						'close'              => __( 'Close', 'accesspress' ),
					),
				),
			),
		);
	...
	private function should_enqueue_icon_picker(): bool {
		...
		return false !== strpos( $screen->id, 'accesspress' )
	...
	public static function get_image( string $file ): string {
		return ImageHelper::get_image_url( 'accesspress-fontawesome', $file );
	}
}
