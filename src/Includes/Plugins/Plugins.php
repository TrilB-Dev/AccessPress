 ...
 * AccessPress - Plugins
 *
 * Handles discovery and loading of AccessPress plugin modules.
 *
 * @package AccessPress
 * @subpackage Includes\AccessPress\Plugins
 * @since 1.0.0
 */

namespace AccessPress\Includes\Plugins;
...
class Plugins {
	...
	public function init(): void {
		...
		do_action( 'accesspress_register_plugin', $this );
	}
	/**
	...
	public function register_plugin_instance( PluginInterface $plugin ): void {
		...
		if ( preg_match( '/-demo$/', $slug ) === 1 ) {
			LoggerHelper::write_log( sprintf( 'AccessPress plugin %s is ignored because plugin slugs ending in -demo are reserved for demo-only plugins.', $slug ) );
			return;
		}
	...
	private function resolve_plugin_directory(): string {
		$path     = trim( Settings::get( 'accesspress_plugin_directory', ACCESSPRESS_PLUGINS ) );
		$resolved = $this->is_absolute_path( $path )
			? untrailingslashit( $path )
			: untrailingslashit( ACCESSPRESS_ROOT ) . '/' . ltrim( str_replace( '\\', '/', $path ), '/' );

		if ( ! is_dir( $resolved ) ) {
			return ACCESSPRESS_PLUGINS;
		}
	...
	private function load_plugin_file( string $file ): void {
		...
		try {
			...
			if ( ! class_exists( $fqcn ) || ! is_a( $fqcn, PluginInterface::class, true ) ) {
				LoggerHelper::write_log( sprintf( 'AccessPress plugin file %s does not declare a PluginInterface implementation.', $file ) );
				return;
			}
			...
			if ( ! $instance instanceof PluginInterface ) {
				LoggerHelper::write_log( sprintf( 'AccessPress plugin %s does not implement PluginInterface.', $fqcn ) );
				return;
			}
			...
			LoggerHelper::write_log( sprintf( 'AccessPress plugin loader failed to require file %s: %s', $file, $e->getMessage() ) );
		}
	}
	/**
	...
	private function initialize_plugin( PluginInterface $plugin ): void {
		...
		try {
			...
			LoggerHelper::write_log( sprintf( 'AccessPress plugin %s failed to initialize: %s', $plugin->get_slug(), $e->getMessage() ) );
		}
	}
	/**
