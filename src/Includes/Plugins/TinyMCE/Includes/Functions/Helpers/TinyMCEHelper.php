 ...
 * @since 1.0.0
 */
namespace AccessPress\Includes\Plugins\TinyMCE\Includes\Functions\Helpers;
...
final class TinyMCEHelper {
	...
	public static function render( string $id, string $name, string $label, string $value = '', int $rows = 8, bool $media_buttons = false, array $options = array() ): void {
		...
		$language_path     = '';
		$language_dir      = dirname( ACCESSPRESS_FILE ) . '/src/Includes/Plugins/TinyMCE/Assets/tinymce/langs';
		...
		if ( file_exists( $language_path ) ) {
			$language_url = ACCESSPRESS_URL . 'src/Includes/Plugins/TinyMCE/Assets/tinymce/langs/' . basename( $language_path );
		}
		...
		$settings = array(
			...
			'skin'          => $ui_skin,
			'skin_url'      => ACCESSPRESS_URL . 'src/Includes/Plugins/TinyMCE/Assets/tinymce/skins/ui/' . $ui_skin,
			'content_css'   => ACCESSPRESS_URL . 'src/Includes/Plugins/TinyMCE/Assets/tinymce/skins/content/' . $content_skin . '/content.min.css',
		...
		$settings = apply_filters( 'accesspress_tinymce_settings', array_merge( $settings, $options ), $editor_id );

		printf( '<label class="form-label" for="%1$s">%2$s</label>', esc_attr( $editor_id ), esc_html( $label ) );
		printf( '<textarea class="form-control accesspress-tinymce" id="%1$s" name="%2$s" rows="%3$d">%4$s</textarea>', esc_attr( $editor_id ), esc_attr( $name ), $rows, esc_textarea( $value ) );
		printf( '<script type="application/json" class="accesspress-tinymce-config" data-editor="%1$s">%2$s</script>', esc_attr( $editor_id ), wp_json_encode( $settings ) );
	}
}
