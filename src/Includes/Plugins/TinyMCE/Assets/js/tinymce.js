(() => {
    ...
    const insertMedia = (editor) => {
        ...
        var frame = window.wp.media({
            title: window.accesspressTinyMCE?.mediaTitle || '',
            button: { text: window.accesspressTinyMCE?.mediaButton || '' },
            multiple: false
        });
    ...
    const initialize = () => {
        ...
        document.querySelectorAll('.accesspress-tinymce-config').forEach((node) => {
            ...
            settings.setup = (editor) => {
                if (settings.media_buttons) {
                    editor.ui.registry.addButton('accesspressmedia', {
                        icon: 'image',
                        tooltip: window.accesspressTinyMCE?.mediaTooltip || '',
            ...
            window.tinymce.init(settings).catch((error) => {
                console.error('AccessPress TinyMCE initialization failed.', error);
            });
        });
    ...
    window.addEventListener('accesspress:settings-tab-loaded', initialize);
