<?php
/**
 * Password field widget for AccessPress forms.
 *
 * @package AccessPress\Includes\Plugins\Elementor\Includes\Widgets\Fields
 * @since 1.0.0
 */
namespace AccessPress\Includes\Plugins\Elementor\Includes\Widgets\Fields;

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

final class PasswordField extends FieldWidget {
	public const SLUG = 'accesspress_password_field';

	protected function get_default_title(): string {
		return __( 'AccessPress Password Field', 'accesspress' );
	}

	protected function field_name(): string {
		return 'password';
	}

	protected function field_type(): string {
		return 'password';
	}
}
