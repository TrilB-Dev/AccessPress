...
namespace AccessPress\Includes\Email;
...
final class Email {
	 ...
	 * wp-content/themes/<theme>/AccessPressEmails/Templates/Admin/<TemplateName>.php
	 *
	...
	private static function resolve_template_class( string $type ): string {
		...
		$namespace  = '\\AccessPress\\Includes\\Email\\Templates';
	...
	private static function resolve_theme_override_class( string $type ): string {
		...
		$class_name = ucfirst( (string) array_pop( $segments ) );
		$namespace  = '\\AccessPressEmails\\Templates';
	...
	public static function theme_override_path( string $type ): string {
		...
		foreach ( array_unique( $theme_roots ) as $theme_root ) {
			$candidate = trailingslashit( $theme_root ) . 'AccessPressEmails/Templates';
			if ( '' !== $path ) {
				$candidate .= '/' . $path;
			}
