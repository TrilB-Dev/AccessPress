<?php
/**
 * Tokenized AccessPress permalink support.
 *
 * @package AccessPress
 */
...
final class PermalinkHelper {
	 ...
	 * @since 1.0.0
	 */
	public const OVERRIDE_META = '_accesspress_permalink';
	/**
	...
	public static function token_definitions(): array {
		return array(
			'%root%'          => __( 'The root AccessPress slug.', 'accesspress' ),
			'%root_category%' => __( 'The Wiki categories, from parent to child.', 'accesspress' ),
			'%root_tags%'     => __( 'The tags assigned to the Wiki container.', 'accesspress' ),
			'%wiki%'          => __( 'The Wiki slug.', 'accesspress' ),
			'%wiki_category%' => __( 'The Wiki page categories, from parent to child.', 'accesspress' ),
			'%wiki_tag%'      => __( 'The tags assigned to the Wiki page.', 'accesspress' ),
			'%wiki_page%'     => __( 'The Wiki page slug.', 'accesspress' ),
		);
	}
	...
	public static function page_url( \WP_Post $page ): string {
		$wiki_id = absint( get_post_meta( $page->ID, '_accesspress_wiki_id', true ) );
		$wiki    = null;
		if ( $wiki_id > 0 ) {
	...
	public static function rewrite_rule(): void {
		add_rewrite_rule( '^(.+?)/?$', 'index.php?accesspress_path=$matches[1]', 'top' );
		add_filter(
			'query_vars',
			static function ( array $vars ): array {
				$vars[] = 'accesspress_path';
				return $vars;
			}
		);
	...
	public static function resolve_request( array $vars ): array {
		$requested_path = isset( $vars['accesspress_path'] ) ? trim( urldecode( (string) $vars['accesspress_path'] ), '/' ) : '';
		if ( '' === $requested_path ) {
			return $vars;
		}
	...
	public static function filter_page_permalink( string $link, \WP_Post $post ): string {
		return $post->post_type === PostType::ACCESSPRESS ? self::page_url( $post ) : $link;
	}
	/**
