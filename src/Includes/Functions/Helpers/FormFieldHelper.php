...
use AccessPress\Includes\Plugins\TinyMCE\Includes\Functions\Helpers\TinyMCEHelper;
...
final class FormFieldHelper {
	...
	public static function dropdown_button( string $label, array $items = array(), array $options = array() ): string {
		...
		if ( ! empty( $options['split'] ) ) {
			$toggle .= '<span class="visually-hidden">' . esc_html__( 'Toggle Dropdown', 'accesspress' ) . '</span>';
		}
	...
	public static function button_toolbar( array $groups = array(), array $options = array() ): string {
		...
		$attributes['id']   = (string) ( $options['id'] ?? sanitize_title( 'accesspress-button-toolbar' ) );
	...
	public static function label( string $for, string $text, array $options = array() ): string {
		...
		if ( ! empty( $options['tooltip'] ) ) {
			...
			$html        .= ' <button type="button" class="btn btn-link p-0 align-baseline accesspress-field-tooltip" data-bs-toggle="tooltip" data-bs-placement="top" title="' . esc_attr( (string) $options['tooltip'] ) . '" aria-label="' . esc_attr( (string) $options['tooltip'] ) . '"><i class="' . esc_attr( $icon ) . '" aria-hidden="true"></i></button>';
		}
