<?php
/**
 * AccessPress encryption helpers.
 *
 * @package AccessPress
 */
...
final class EncryptionHelper {
	...
	public static function ensure_configured(): bool {
		...
		if ( null === $config_path || ! is_readable( $config_path ) || ! is_writable( $config_path ) ) {
			...
			error_log( 'AccessPress: wp-config.php was not found or is not writable; encryption key was not created.' );
			return false;
		}
		...
		if ( false === $config_content ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
			error_log( 'AccessPress: could not read wp-config.php; encryption key was not created.' );
			return false;
		}

		if ( preg_match( '/define\s*\(\s*[\'\"]ACCESSPRESS_ENCRYPTION_KEY[\'\"]\s*,/i', $config_content ) ) {
			return true;
		}
		...
		try {
			...
			error_log( 'AccessPress: failed to generate the encryption key: ' . $exception->getMessage() );
			return false;
		}

		$new_line        = "define( 'ACCESSPRESS_ENCRYPTION_KEY', '" . addslashes( $key ) . "' );" . PHP_EOL;
		...
		if ( false !== $marker_position ) {
			...
			if ( false === $settings_position ) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
				error_log( 'AccessPress: could not find a safe insertion point in wp-config.php.' );
				return false;
			}
		...
		if ( false === $handle || ! flock( $handle, LOCK_EX ) ) {
			...
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
			error_log( 'AccessPress: could not lock wp-config.php; encryption key was not created.' );
			return false;
		}
		...
		if ( ! $success ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- This is only used when the config file cannot be updated safely.
			error_log( 'AccessPress: could not write the encryption key to wp-config.php.' );
			return false;
		}

		if ( ! defined( 'ACCESSPRESS_ENCRYPTION_KEY' ) ) {
			define( 'ACCESSPRESS_ENCRYPTION_KEY', $key );
		}
	...
	private static function runtime_key(): ?string {
		...
		return (string) constant( 'ACCESSPRESS_ENCRYPTION_KEY' );
	}
	/**
