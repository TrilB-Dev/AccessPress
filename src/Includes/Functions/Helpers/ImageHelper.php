...
final class ImageHelper {
	...
	public static function get_image_url( string $type, string $file ): string {
		...
		if ( 'core' === strtolower( trim( $type ) ) ) {
			return ACCESSPRESS_ASSETS_URL . '/images/' . $file;
		}
		...
		return ACCESSPRESS_PLUGINS_URL . '/' . $plugin_directory . '/Assets/images/' . $file;
	}
	...
	private static function get_plugin_directory( string $plugin_slug ): string {
		...
		if ( '' === $plugin_slug || ! defined( 'ACCESSPRESS_PLUGINS' ) || ! is_dir( ACCESSPRESS_PLUGINS ) ) {
			return '';
		}

		$plugin_slug_variants = array( $plugin_slug );
		if ( str_starts_with( $plugin_slug, 'accesspress-' ) ) {
		...
		$directories = scandir( ACCESSPRESS_PLUGINS );
		if ( false === $directories ) {
			return '';
		}

		foreach ( $directories as $directory ) {
			if ( '.' !== $directory && '..' !== $directory && is_dir( ACCESSPRESS_PLUGINS . '/' . $directory ) && in_array( strtolower( $directory ), $plugin_slug_variants, true ) ) {
				return $directory;
			}
		}
