<?php
/**
 * AccessPress - Functions
 *
 * Shared utilities used by AccessPress services, pages, and REST routes.
 *
 * @package TrilBDev
 * @subpackage Includes\AccessPress\Functions
 * @since 1.0.0
 */

namespace AccessPress\Includes\Functions;
...
final class Functions {
	/**
	 * Default status for AccessPress posts.
	 */
	public const DEFAULT_STATUS = 'publish';
	/**
	 * Allowed statuses for AccessPress posts.
	 */
	public const ALLOWED_STATUSES = array( 'publish', 'draft', 'private' );
	/**
	 * Sanitizes a AccessPress payload array for safe use.
	 *
	...
	/**
	 * Checks if a given post is a AccessPress post.
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a AccessPress post, false otherwise.
	 */
	public static function is_post( $post ): bool {
		return self::is_page( $post );
	}
	/**
	 * Checks if a given post is a AccessPress page.
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a AccessPress page, false otherwise.
	 */
	public static function is( $post ): bool {
		return PostHelper::is( $post );
	}
	/**
	 * Checks if a given post is a AccessPress content (either a AccessPress post or a AccessPress page).
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a AccessPress content, false otherwise.
	 */
	public static function is_page( $post ): bool {
		return PostHelper::is_page( $post );
	}
	/**
	 * Checks if a given post is either a AccessPress post or a AccessPress page.
	 *
	 * @param mixed $post The post to check.
	 * @return bool True if the post is a AccessPress content, false otherwise.
	 */
