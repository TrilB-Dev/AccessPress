...
final class I18n {
	/**
	 * Default Local Translation of AccessPress
	 * 
	 * @since 1.0.0
	 */
	 ...
	 */
	public function __construct( string $domain = 'accesspress', ?string $languages_path = null, ?string $plugin_file = null, string $default_locale = self::DEFAULT_LOCALE ) {
		$this->domain         = sanitize_key( $domain );
		$plugin_basename      = defined( 'ACCESSPRESS_BASENAME' ) ? ACCESSPRESS_BASENAME : ( $plugin_file && function_exists( 'plugin_basename' ) ? plugin_basename( $plugin_file ) : null );
