document.addEventListener('DOMContentLoaded', () => {
    ...
    const showPluginNotice = (alertMarkup) => {
        const container = root.querySelector('#accesspress-settings-panel .accesspress-settings-tab-content');
        if (!container || !alertMarkup) return;

        container.querySelectorAll('[data-accesspress-alert]').forEach((notice) => notice.remove());
    ...
    const togglePlugin = (toggle) => {
        ...
        const body = new URLSearchParams({
            action: 'accesspress_toggle_plugin',
    ...
    const savePluginSettings = (button) => {
        const modal = button.closest('.accesspress-plugin-settings-modal');
        ...
        body.set('action', 'accesspress_save_plugin_settings');
    ...
    root.addEventListener('click', (event) => {
        ...
        // Close modal
        const dismiss = event.target.closest?.('.accesspress-plugin-settings-modal [data-bs-dismiss="modal"]');
        if (dismiss) {
            event.preventDefault();
            const modal = dismiss.closest('.accesspress-plugin-settings-modal');
    ...
    root.addEventListener('change', (event) => {
        const toggle = event.target.closest?.('[data-accesspress-plugin-toggle]');
        if (toggle) {
            togglePlugin(toggle);
        }
    }, true);
