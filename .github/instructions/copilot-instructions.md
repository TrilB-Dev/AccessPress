# AccessPress Architecture Instructions

AccessPress is a modular WordPress platform. Internal plugins and separately installed WordPress plugins are the primary way to add features and integrations. Preserve this architecture in every change.
...
- `src/languages/` contains AccessPress core translation catalogs and compiled language files.
...
- Keep feature-specific behavior out of AccessPress core. Core must not contain conditions, selectors, settings keys, labels, capabilities, or business rules for a particular plugin.
...
- Internal plugins live under `src/Includes/Plugins/<AccessPress>/` and are initialized by `Plugins.php` through the contracts in `PluginsInterface.php`.
- Separately installed plugins register through `accesspress_register_plugin` and follow the same ownership rules. They must not require core changes that reference their slug or class.
- Do not bypass the plugin loader with direct feature requires from `AccessPress.php`, `Plugin.php`, or core bootstrap code.
...
All AccessPress and internal-plugin assets must flow through `src/Assets/Assets.php`.

- Plugin asset classes may define asset lists and page conditions, but must register them through the central `AccessPress\Assets\Assets` service.
...
- All direct WordPress database access must route through the shared AccessPress database wrapper in `src/Includes/Core/WP/Database.php`.
- Feature classes must not call `$wpdb` directly; use the centralized helper facade in `src/Includes/Functions/Helpers/DBHelper.php` for all read/write operations.
- Keep schema and table-name resolution centralized in `Database::table_name()` and related helpers so all custom tables follow the `wp_accesspress_<suffix>` convention.
- Settings and custom data operations should go through `SettingsManager` / `Settings` or the DB helper instead of ad hoc SQL scattered across feature code.
- The default settings table remains `accesspress_settings` with the default group row `accesspress_general` unless a specific stored-table contract is intentionally being used.
- Additional custom tables such as `groups` and `forms` must follow the same routed pattern through the database abstraction rather than raw SQL in plugin classes.
- Do not add new raw database access in feature modules or helpers when the shared DB abstraction already covers the operation.
...
- AccessPress and its plugins use the repository language scripts to manage translation catalogs and compiled language files.
