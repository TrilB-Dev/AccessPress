...
A AccessPress extension implements `PluginInterface`, returns a unique slug, and exposes metadata and an `init()` method. Optional capability interfaces add settings, assets, admin pages, REST routes, frontend behavior, database tables, shortcodes, or translations.

Internal extensions are discovered from the configured AccessPress plugin directory. A normal WordPress plugin can register an extension by hooking `accesspress_register_plugin` and calling `Plugins::register_plugin()`.
...
AccessPress is distributed under the license declared by the project and its individual dependencies. Confirm the applicable license before redistributing a packaged build.
