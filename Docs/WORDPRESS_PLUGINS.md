...
A separately installed WordPress plugin can extend AccessPress without being placed in the internal extension directory. The bridge is the `accesspress_register_plugin` action.
...
<?php
/**
 * Plugin Name: My AccessPress Extension
 */

use AccessPress\Includes\Plugins\Plugins;
use AccessPress\Includes\Plugins\PluginInterface;

add_action('accesspress_register_plugin', static function (Plugins $plugins): void {
...
final class MyPluginExtension implements PluginInterface {
    ...
    public function get_description(): string { return 'Adds a AccessPress integration.'; }
...
Use `PermissionHelper` for protected routes and import `Response` from `AccessPress\API` when using the host response envelope. Shortcode callbacks must return rendered output.
...
Implement settings providers for defaults and generated settings pages. Read through `AccessPress\Includes\Settings\Settings` and sanitize all submitted values before storing them. External extensions own their translation catalogs and language files.
...
1. Declare AccessPress as a dependency or check for its classes.
2. Load the extension class before the registration hook runs.
3. Register on `accesspress_register_plugin`.
...
7. Test activation with AccessPress active and inactive.
