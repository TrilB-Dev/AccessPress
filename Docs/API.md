...
```php
use AccessPress\API\ResourceService;
...
Define narrowly scoped, documented hooks when another plugin needs to change a payload or observe a domain event. Use the plugin slug as the prefix, for example `accesspress_resource_payload` or `accesspress_resource_saved`. Apply authorization before exposing data, and document whether hooks run before or after persistence.
